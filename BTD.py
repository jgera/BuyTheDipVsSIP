...
SIP_Interval = 4  # Invest Every 4th week approx end of month
max_trades_interval = 4
SIP_Amount = 10000 / max_trades_interval

trades = []
Month_Trades = []
week_idx = 1

def CanBuy(Dip, Monthly_trades, week_idx, max_trades_interval = 4):
    

    # Return (False, 0) if max trades reached
    if len(Monthly_trades) >= max_trades_interval:
        return False, 0
    
    # If no trades in 4 weeks, ensure buy_amount is 4 at month-end (assuming week_idx can be 4 or higher)
    if len(Monthly_trades) == 0 and week_idx >= 4:
        return True, 4
    
    # Buy only if dip condition is met
    if not Dip and week_idx <= 3:
        return False, 0

    # Calculate buy_amount based on weeks without a trade
    weeks_without_trade = max(0, week_idx - len(Monthly_trades) - 1)
    buy_amount = 1 + weeks_without_trade
...
for index, row in data.iterrows():
    ...
    can_buy, buy_amount  = CanBuy(Dip, Month_Trades, week_idx,max_trades_interval = max_trades_interval)
    

    if can_buy:
        print(f"Week {week_idx} | {str(index.date())} : Buying {len(Month_Trades)+1} time in the month")
        volume = SIP_Amount / row['close']
